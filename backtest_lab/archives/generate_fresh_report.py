    ...
    def run_simulation(self, stock_data_dict):
        ...
        for i in range(45, max_len):
            ...
            for name in stock_names:
                ...
                # 🛡️ FRESHNESS & HARMONY
                is_aligned = (row['ema5_15m'] < row['ema9_15m']) and (row['ema20_15m'] < row['ema50_15m'])
                is_fresh = (row['adx_15m'] > 20) and (row['adx_15m'] > prev['adx_15m'])
                
                orb = df.attrs['orb']
                if is_aligned and is_fresh and row['close'] < orb['l']:
                    candidates.append({'name': name, 'side': 'SHORT', 'ltp': row['close'], 'time': ts})
...
def run_fresh_study(target_date_str):
    ...
    engine = FreshEliteEngine(top_n=5) # Increased to Top 5
    engine.run_simulation(stock_data)
    df_res = pd.DataFrame(engine.all_trades)
    
    html_file = f"python-trader/backtest_lab/reports/fresh_elite_report_{target_date_str}.html"
    total_pnl = pd.to_numeric(df_res['PnL_Rs'], errors='coerce').sum() if not df_res.empty else 0
    with open(html_file, "w") as f:
        f.write(f"<html><body class='p-5'><h1>🛡️ Fresh Elite Report (Top 5 | ADX Rising Check)</h1><h2>Total PnL: Rs {total_pnl:,.2f}</h2>{df_res.to_html(classes='table')}</body></html>")
    print(f"✅ Fresh Elite Report generated: {html_file}")

if __name__ == "__main__":
    run_fresh_study("2026-01-21")
