...
class InstitutionalHighConvictionEngine:
        ...
        for i in range(45, max_len):
            ...
            for name, pos in self.active_positions.items():
                ...
                if exit_hit or ts.time() >= dt_time(15, 15):
                    self.all_trades.append({'Time': ts, 'Stock': name, 'Action': 'SQUARE-OFF', 'PnL_Rs': round(pnl_rs, 2), 'Reason': 'FAST_EXIT' if exit_hit else 'EOD'})
                    to_close.append(name)
            for name in to_close: del self.active_positions[name]

            # ENTRY (15m only | ADX > 25)
            ...
            for name in stock_names:
                ...
                # 🛡️ STRONG TREND FILTER (ADX > 25)
                is_long = (row['ema5_15m'] > row['ema9_15m']) and (row['ema20_15m'] > row['ema50_15m']) and (row['adx_15m'] > 25)
                is_short = (row['ema5_15m'] < row['ema9_15m']) and (row['ema20_15m'] < row['ema50_15m']) and (row['adx_15m'] > 25)
...
def generate_high_conviction_matrix():
    ...
    engine = InstitutionalHighConvictionEngine(top_n=5)
    
    for d in all_dates:
        print(f"▶️ Simulating: {d} (ADX > 25)")
        stock_data_day = {}
        for s in top_stocks:
    ...
    df_matrix.to_html("python-trader/backtest_lab/reports/high_conviction_matrix.html")
    print(f"✅ High Conviction Matrix generated: python-trader/backtest_lab/reports/high_conviction_matrix.html")

if __name__ == "__main__":
    generate_high_conviction_matrix()
