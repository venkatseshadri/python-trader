    ...
    def run_simulation(self, stock_data_dict):
        ...
        for i in range(45, max_len):
            ...
            for name in stock_names:
                ...
                # Entry Alignment (Triple Timeframe)
                is_long = (row['ema5_5m'] > row['ema9_5m']) and (row['ema5_15m'] > row['ema9_15m']) and (row['ema20_5m'] > row['ema50_5m'])
                is_short = (row['ema5_5m'] < row['ema9_5m']) and (row['ema5_15m'] < row['ema9_15m']) and (row['ema20_5m'] < row['ema50_5m'])
...
def run_momentum_study(target_date_str):
    ...
    engine = MomentumCaptureEngine(top_n=5)
    engine.run_simulation(stock_data)
    df_res = pd.DataFrame(engine.all_trades)
    if df_res.empty:
        print("Still no trades. Relaxation logic may be needed.")
        return
    total_pnl = pd.to_numeric(df_res['PnL_Rs'], errors='coerce').sum()
    html_file = f"python-trader/backtest_lab/reports/momentum_15m_exit_{target_date_str}.html"
    with open(html_file, "w") as f:
        f.write(f"<html><body class='p-5'><h1>🚀 Momentum Capture (15m Structural Exit)</h1><h2>Day PnL: Rs {total_pnl:,.2f}</h2>")
        f.write(df_res.to_html(classes='table'))
        f.write("</body></html>")
    print(f"✅ 15m Momentum Report generated: {html_file}")

if __name__ == "__main__":
    run_momentum_study("2026-01-21")
