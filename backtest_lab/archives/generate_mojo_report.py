    ...
    def run_simulation(self, stock_data_dict):
        ...
        for i in range(45, max_len):
            ...
            for name in stock_names:
                ...
                is_mojo_ok = (row['adx_15m'] > adx_threshold) and (row['adx_15m'] > prev['adx_15m']) and (rel_atr > atr_threshold)
                
                if not is_mojo_ok: continue
...
def run_mojo_shield_study(target_date_str):
    ...
    print(f"🚀 Running Mojo Shield Study (Dynamic ADX/ATR)...")
    engine = MojoShieldEngine()
    engine.run_simulation(stock_data)
    df_res = pd.DataFrame(engine.all_trades)
    
    html_file = f"python-trader/backtest_lab/reports/mojo_shield_report_{target_date_str}.html"
    pnl = pd.to_numeric(df_res['PnL_Rs'], errors='coerce').sum() if not df_res.empty else 0
    with open(html_file, "w") as f:
        f.write(f"<html><body class='p-5'><h1>🛡️ Mojo Shield Report (Dynamic Intensity Guards)</h1><h2>Total PnL: Rs {pnl:,.2f}</h2>{df_res.to_html(classes='table')}</body></html>")
    print(f"✅ Mojo Report generated: {html_file}")

if __name__ == "__main__":
    run_mojo_shield_study("2026-01-21")
