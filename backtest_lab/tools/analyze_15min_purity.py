...
OUTPUT_FILE = "backtest_lab/data/15min_ema_purity.csv"
...
def analyze_15min_purity():
    ...
    results = []
    print(f"🚀 Analyzing 15-min Trend Purity for {len(elite_symbols)} stocks...")
        ...
        for f_name in json_files:
            try:
                ...
                for p in ema_periods:
                    if len(df_15m) < p: continue
                    ema_col = f"ema{p}"
                    df_15m[ema_col] = talib.EMA(df_15m['close'].values.astype(float), timeperiod=p)
                    breaks = count_breaks(df_15m.dropna(), ema_col, direction)
    ...
    if results:
        ...
        print("\n🏆 15-MIN TREND PURITY RANKING (Lowest Breaks = Best):")
        print("-" * 60)
        print(summary.sort_values().to_string())

if __name__ == "__main__":
    analyze_15min_purity()
