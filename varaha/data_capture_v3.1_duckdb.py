...
class IndicatorBuffer:
    ...
    def warmup_from_log(self, log_path: str, max_bars: int = 200):
        """Rebuild buffer from OHLCV log file after a restart, so ta-lib
        indicators compute immediately without waiting for 14+ fresh bars."""
        try:
            import json
...
def capture_once(
    ...
    redis_client: Optional[object] = None,  # [v3.1] Redis client for queue publishing
    log_file: Optional[object] = None,  # [v3.1] Log file handle for backup
    indicators_log: Optional[object] = None,  # [v3.1] Indicator redundancy log
) -> Tuple[Dict, bool]:
    now = datetime.now()
    ...
    # [v3.1] Write indicators to redundancy log for buffer recovery on restart
    if indicators_log:
        try:
            entry = {"timestamp": timestamp}
            entry.update(
                {k: v for k, v in indicators.items() if v is not None and k != "vwap"}
            )
            indicators_log.write(json.dumps(entry) + "\n")
            indicators_log.flush()
        except Exception:
            pass
    ...
    if spot:
        ...
        # [v3.1 ENHANCEMENT] Publish 1-min OHLCV to Redis queue + log file for v4 aggregator
        ohlcv_bar = {
            "timestamp": timestamp,
            "index": ds.index,
            "open": spot,
        ...
        # Publish to Redis queue (primary path)
        if redis_client:
            try:
                redis_client.lpush("v3_ohlcv_queue", json.dumps(ohlcv_bar))
            except Exception as e:
                logger.debug(f"[v3.1] Redis publish failed: {e}")

        # Write to log file (backup path)
        if log_file:
            try:
                log_file.write(json.dumps(ohlcv_bar) + "\n")
                log_file.flush()
            except Exception as e:
                logger.debug(f"[v3.1] Log file write failed: {e}")
...
def run_capture_loop(use_broker: bool = True, once: bool = False, index: str = "NIFTY"):
    ...
    # Warm up buffer from OHLCV log so ta-lib indicators compute immediately
    log_dir_warmup = Path("/home/trading_ceo/brahmand/logs")
    ohlcv_log = log_dir_warmup / f"v3_ohlcv_{index}.log"
    if ohlcv_log.exists():
        buf.warmup_from_log(str(ohlcv_log))
        logger.info(f"🔥 Warmup: buffer loaded {len(buf.buf)} bars from {ohlcv_log}")
    ...
    # Initialize Redis for queue publishing (v3.1 enhancement)
    redis_client = None
    try:
        ...
        redis_client.ping()
        logger.info("✅ [v3.1] Redis queue connected")
    except Exception as e:
        logger.warning(
            f"⚠️ [v3.1] Redis connection failed: {e}, will skip queue publishing"
        )

    # Initialize log file for backup (v3.1 enhancement)
    log_file = None
    try:
        log_dir = Path("/home/trading_ceo/brahmand/logs")
        log_dir.mkdir(parents=True, exist_ok=True)
        log_file_path = log_dir / f"v3_ohlcv_{index}.log"
        log_file = open(log_file_path, "a")
        logger.info(f"✅ [v3.1] Log file opened: {log_file_path}")
    except Exception as e:
        logger.warning(f"⚠️ [v3.1] Log file failed: {e}")

    # Initialize indicators redundancy log (v3.1)
    indicators_log_file = None
    try:
        log_dir = Path("/home/trading_ceo/brahmand/logs")
        log_dir.mkdir(parents=True, exist_ok=True)
        ind_log_path = log_dir / f"v3_indicators_{index}.log"
        indicators_log_file = open(ind_log_path, "a")
        logger.info(f"✅ [v3.1] Indicators log opened: {ind_log_path}")
    except Exception as e:
        logger.warning(f"⚠️ [v3.1] Indicators log failed: {e}")
    ...
    while True:
        ...
        try:
            snap, captured_daily = capture_once(
                ...
                redis_client=redis_client,  # [v3.1]
                log_file=log_file,  # [v3.1]
                indicators_log=indicators_log_file,  # [v3.1]
            )
            if captured_daily:
                daily_done = True
            ...
            logger.error(
                f"ERROR in capture_once: {type(e).__name__}: {e}", exc_info=True
            )
        finally:
        ...
        # Reconnect for next write cycle with retry on lock conflict
        for retry in range(3):
            try:
                db = duckdb.connect(str(DB_PATH))
                break
            except Exception as e:
                logger.warning(f"DuckDB reconnect failed (attempt {retry + 1}/3): {e}")
                time.sleep(5)
        else:
            logger.error("DuckDB reconnect failed after 3 retries, reinitializing...")
            time.sleep(10)
            db = duckdb.connect(str(DB_PATH))
            init_schema(db)
