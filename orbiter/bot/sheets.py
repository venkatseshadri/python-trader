...
def log_scan_metrics(metrics, tab_name="scan_metrics"):
    ...
    for item in metrics:
        ...
        values = {
            ...
            "Day Open": f"₹{try_float(item.get('day_open')):.2f}",
            "Day High": f"₹{try_float(item.get('day_high')):.2f}",
            "Day Low": f"₹{try_float(item.get('day_low')):.2f}",
            "Day Close": f"₹{try_float(item.get('day_close')):.2f}",
            "ORB Open": f"₹{try_float(orb.get('orb_open')):.2f}",
            "EMA5": f"₹{try_float(ema.get('ema5')):.2f}",
            "ORB High": f"₹{try_float(orb.get('orb_high')):.2f}",
