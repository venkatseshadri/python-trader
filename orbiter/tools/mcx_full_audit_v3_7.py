...
def get_futures_token(symbol):
    ...
    if (not res or 'values' not in res) and symbol == 'GOLDPETAL':
        res = api.searchscrip(exchange='MCX', searchtext='PETAL')
    ...
    if res and 'values' in res:
        ...
        for item in valid:
            ...
            if tsym.startswith(symbol) or (symbol == 'GOLDPETAL' and 'GOLDPETAL' in tsym):
...
def run_audit():
    ...
    print(f"✅ Logged in. Auditing Portfolio (Date: {datetime.date.today()})\n")
    print(f"{'Symbol':<22} | {'Token':<8} | {'LTP':<10} | {'ADX':<6} | {'Status'}")
    print("-" * 75)
    ...
    for sym in SYMBOLS:
        ...
        if not token:
            print(f"{sym:<22} | {'-':<8} | {'-':<10} | {'-':<6} | ⚪ NOT FOUND")
            continue
        ...
        if q and q.get('stat') == 'Ok' and isinstance(h, list) and len(h) > 25:
            try:
                ...
                print(f"{tsym:<22} | {token:<8} | {lp:<10.2f} | {adx_val:<6.2f} | {status}")
                
                if adx_val > 25: trending.append((tsym, adx_val))
                else: choppy.append((tsym, adx_val))
            except Exception as e:
                print(f"{tsym:<22} | ERROR in Calculation")
        else:
            print(f"{tsym:<22} | {'NO DATA':<8} | {'-':<10} | {'-':<6} | ❌ INACTIVE")
