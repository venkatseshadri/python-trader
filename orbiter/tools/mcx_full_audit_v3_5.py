...
def run_audit():
    ...
    print(f"✅ Logged in. Analyzing {len(SYMBOLS)} symbols...\n")
    ...
    for sym in SYMBOLS:
        ...
        if q and q.get('stat') == 'Ok' and isinstance(h, list) and len(h) > 25:
            try:
                ...
                print(f"{tsym:<20} | {token:<8} | {float(q['lp']):<8.2f} | {adx:<6.2f} | {status}")
                
                if adx > 25: trending.append((tsym, adx))
                else: choppy.append((tsym, adx))
            except: print(f"{tsym:<20} | Calculation Error")
        else: print(f"{tsym:<20} | {'INACTIVE':<8}")
        time.sleep(0.1)
