...
class TestScanLogic(unittest.TestCase):
    def setUp(self):
        ...
        # Mock State with properly nested structure
        self.state = MagicMock()
        self.state.symbols = ['NFO|12345']
        self.state.config = {'SIMULATION': True}  # Add config
        self.state.filter_results_cache = {
            'NFO|12345': {'f1': {'score': 10.5}}
        }
        
        # Set up the nested mock for SYMBOLDICT - this is what the code accesses
        self.state.client = MagicMock()
        self.state.client.conn = MagicMock()
        self.state.client.conn.tick_handler = MagicMock()
        self.state.client.conn.tick_handler.SYMBOLDICT = {}

    def _get_report_for_prices(self, ltp, open_price, prev_close=None):
        """Helper to inject prices and get the report string"""
        token = 'NFO|12345'
        
        # Set up SYMBOLDICT with the prices
        self.state.client.conn.tick_handler.SYMBOLDICT = {
            token: {
                'symbol': 'TESTSTOCK',
                'tsym': 'TESTSTOCK24FEB26F',
                'lp': str(ltp),
                'o': str(open_price),
